...
/*bar1:必须与对象名相同
  网上图书商城：大标题
*/
var bar1 = new Q6MenuBar("bar1", "网上图书商城");
function load() {
	bar1.colorStyle = 2; //设置配色方案，一共4种（0,1,2,3）
	bar1.config.imgDir = "<c:url value='/menu/img/'/>"; //指定图片目录
	bar1.config.radioButton=false; //菜单之间是否相互排斥(排斥即为打开这个菜单，其他菜单就关闭了)
	/*分类管理：指定要添加的菜单名称（若这个名称的菜单已经存在，不会重复添加）
		查看分类：指定要添加的菜单项名称
	...
	d.innerHTML = bar1.toString(); //把菜单对象转换成字符串，赋给<div>元素做内容
}
</script>
